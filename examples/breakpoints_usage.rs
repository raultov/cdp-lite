...
async fn main() -> CdpResult<()> {
    ...
    let cdp_client = cdp_lite::client::CdpClient::new("127.0.0.1:9222", Duration::from_secs(200)).await?;
    let cdp_client_clone = cdp_client.clone();
    cdp_client.send_raw_command("Page.enable", NoParams).await?;
    cdp_client.send_raw_command("Debugger.enable", NoParams).await?;
                ...
                if let Some(method) = method {
                    match method {
                        "Debugger.scriptParsed" => {
                            ...
                            let script_id = extract_from_value(&event.params, "scriptId").ok_or_else(|| CdpError::InternalError("Script Id not found".to_string()))?;
                            if let Ok(script_result) = cdp_client_clone.send_raw_command("Debugger.getScriptSource", json!({"scriptId": script_id})).await {
                                if let Some((line_number, column_number)) = extract_from_value(&script_result.result, "scriptSource")
                                    .and_then(|s| find_line_column(s, "nav_dropdown.value")) {
                                    let hash = extract_from_value(&event.params, "hash").ok_or_else(|| CdpError::InternalError("Hash Id not found".to_string()))?;

                                    cdp_client_clone.send_raw_command("Debugger.setBreakpointByUrl", json!({
                                    ...
                                    target_found = true;
                                    cdp_client_clone.send_raw_command("Page.reload", NoParams).await?;
                                }
                            }
                        }
                        "Debugger.paused" => {
                            let (call_frame_id, function_name) = event.params.as_ref()
                                .and_then(|p| p.get("callFrames"))
                                .and_then(|frames| frames.as_array())
                                .and_then(|frames| frames.first())
                                .and_then(|first_frame| {
                                    first_frame.get("callFrameId").and_then(|id| id.as_str())
                                        .zip(first_frame.get("functionName").and_then(|name| name.as_str()))
                                })
                                .ok_or_else(|| CdpError::InternalError("Frame Id or Function Name not found".into()))?;

                            let expression_result = cdp_client_clone.send_raw_command("Debugger.evaluateOnCallFrame", json!({
                                "callFrameId": call_frame_id,
                                "returnByValue": true,
                                "expression": "current_lang"
                            })).await?;

                            info!("Expression result: {:?} from Function Name: {}", expression_result, function_name);

                            cdp_client_clone.send_raw_command("Debugger.resume", NoParams).await?;
    ...
    let result = timeout(Duration::from_secs(10), expression_evaluated_signal.notified()).await;
    match result {
