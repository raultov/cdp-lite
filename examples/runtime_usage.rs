...
async fn main() -> CdpResult<()> {
    ...
    cdp_client.send_raw_command("Runtime.enable", NoParams).await?;
    cdp_client.send_raw_command("Page.navigate", json!({"url": "https://www.rust-lang.org"})).await?;

    let expression_result = cdp_client.send_raw_command("Runtime.evaluate", json!({
        "expression": "({
    ...
    let expression_promise_result = cdp_client.send_raw_command("Runtime.evaluate", json!({
        "expression": "  (async () => {
            const r = await fetch('/');
            return r.status;
        })()",
    ...
    info!("Expression promise outcome: {:?}", expression_promise_result);
